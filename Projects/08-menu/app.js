...
// dynamic filter buttons --> reduce()
function displayMenuCategories(){
  let categories = menu.reduce((values, item)=>{
    if(!values.includes(item.category)){  // for unique category
  ...
  // after adding dynamically, you can access filter-btn class afterwards only.
  const filterBtns = document.querySelectorAll('.filter-btn');
